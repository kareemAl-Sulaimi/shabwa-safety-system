...
export default function AdminLostPage() {
  ...
  const loadData = async () => {
    try {
      setLoading(true)
      const data = await api.admin.lostReports()
      setItems(data)
    } catch (err) {
  ...
  return (
    <div className="admin-page">
      <div className="admin-page__header">
        ...
        <h1 className="page-heading__title">إدارة بلاغات المفقودات</h1>
      </div>
            ...
            {items.map(item => (
              <tr key={item.id} className={item.deleted_at ? 'row--deleted' : ''}>
                ...
                <td className="td-actions">
                  {item.state === 'ACTIVE' && (
                    <>
                      <button className="btn btn--success btn--xs" onClick={() => handleStateChange(item.id, 'found')}>وجد</button>
